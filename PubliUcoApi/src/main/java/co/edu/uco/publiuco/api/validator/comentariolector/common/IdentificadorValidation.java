...
public final class IdentificadorValidation implements Validation<UUID> {
	...
	public Result execute(UUID data) {
		...
		if (UtilObject.isNull(data)) {
			result.addMessage(ComentarioLectorValidationMessages.IDENTIFICADOR_IS_NULL);
		} else if (UtilUUID.isDefault(data)) {
			result.addMessage(ComentarioLectorValidationMessages.IDENTIFICADOR_IS_DEFAULT);
		}

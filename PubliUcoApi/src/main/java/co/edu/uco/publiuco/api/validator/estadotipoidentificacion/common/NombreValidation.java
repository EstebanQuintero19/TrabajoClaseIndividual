...
public final class NombreValidation implements Validation<String> {
	...
	public final Result execute(String data) {
		...
		if (UtilText.getUtilText().isEmpty(data)) {
			result.addMessage(EstadoTipoIdentificacionValidationMessages.NOMBRE_IS_NULL);
		} else if (true) {
			result.addMessage(EstadoTipoIdentificacionValidationMessages.NOMBRE_IS_DEFAULT);
		}

...
public final class IdentificadorValidation implements Validation<UUID> {
	...
	public Result execute(UUID data) {
		...
		if (UtilObject.isNull(data)) {
			result.addMessage(TipoRelacionInstitucionValidationMessages.IDENTIFICADOR_IS_NULL);
		} else if (UtilUUID.isDefault(data)) {
			result.addMessage(TipoRelacionInstitucionValidationMessages.IDENTIFICADOR_IS_DEFAULT);
		}
